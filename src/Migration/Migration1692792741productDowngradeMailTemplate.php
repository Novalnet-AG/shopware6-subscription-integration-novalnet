...
class Migration1692792741productDowngradeMailTemplate extends MigrationStep
{
    ...
    public function createProductChangeTemplate(Connection $connection): void
    {
        ...
        $mailTypeId = $this->getMailTypeMapping()['novalnet_product_downgrade_mail']['id'];
        $deLangId = $enLangId = '';
        ...
        if (!$this->checkMailType($connection)) {
            $connection->insert(
                ...
                'technical_name' => 'novalnet_product_downgrade_mail',
                'available_entities' => json_encode($this->getMailTypeMapping()['novalnet_product_downgrade_mail']['availableEntities']),
            ...
            if ($enLangId != '') {
                $connection->insert(
                    ...
                    [
                        ...
                        'subject' => 'The product quantity has been downgraded for your subscription order number {{ subs.subsNumber }}',
                        'description' => 'Subscription Product Downgrade Mail',
                ...
                $connection->insert(
                    ...
                    [
                        'mail_template_type_id' => Uuid::fromHexToBytes($mailTypeId),
                        'language_id' => Uuid::fromHexToBytes($enLangId),
                        'name' => 'Subscription Product Downgrade Mail',
                        'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT),
                    ]
                );
            }
            ...
            if ($deLangId != '') {
                $connection->insert(
                    ...
                    [
                        ...
                        'subject' => 'Die Produktmenge wurde für Ihre Abonnementbestellnummer herabgestuft {{ subs.subsNumber }}',
                        'description' => 'Abonnement Produkt Downgrade Mail',
                        'sender_name' => '{{ salesChannel.name }}',
                ...
                $connection->insert(
                    ...
                    [
                        'mail_template_type_id' => Uuid::fromHexToBytes($mailTypeId),
                        'language_id' => Uuid::fromHexToBytes($deLangId),
                        'name' => 'Abonnement Produkt Downgrade Mail',
                        'created_at' => (new \DateTime())->format(Defaults::STORAGE_DATE_TIME_FORMAT),
                    ]
                );
            }
    ...
    {
        return[
            'novalnet_product_downgrade_mail' => [
                'id' => Uuid::randomHex(),
                'name' => 'Subscription Product Downgrade Mail',
                'nameDe' => 'Abonnement Produkt Downgrade Mail',
...
<div style="font-family:Arial, Helvetica, sans-serif; font-size:12px;">
	<p>
		...
		<br>
		The product quantity has been downgrade for your subscription (Subscription Number: {{subs.subsNumber}}) from the shop {{ salesChannel.name }}.<br>
		<br>
		...
		<br>
		<br>
		Hereafter, the recurring order will be processed with updated product quantity.
		<br>
		<br>
...
<div style="font-family:Arial, Helvetica, sans-serif; font-size:12px;">
	<p>
		...
		{{order.orderCustomer.salutation.letterName }} {{order.orderCustomer.firstName}} {{order.orderCustomer.lastName}},<br>
		<br>
		Die Produktmenge wurde für Ihr Abonnement herabgestuft (Abonnementnummer: {{subs.subsNumber}}) aus dem Geschäft {{ salesChannel.name }}.<br>
		<br>
		...
		<br>
		<br>
		Danach wird die wiederkehrende Bestellung mit aktualisierten Produktmenge.
		<br>
		<br>
